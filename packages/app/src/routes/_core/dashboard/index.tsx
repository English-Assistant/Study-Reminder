...
import { Typography, App, Spin, Tabs, Divider, Badge, Tag } from 'antd';
...
function DashboardComponent() {
  ...
  return (
    <Spin spinning={loading} tip="加载中...">
      <div className="grid grid-cols-[30vw_1fr] gap-6">
        <div className="bg-#fff rounded-3 p-6">
          <Title level={2}>学习统计</Title>

          <div className="flex gap-4">
            <div className="rounded-2 bg-#F3F3FF p-4 flex-1">
              <div className="color-#4B5563 text-size-4 lh-5">连续打卡</div>
              <div className="color-#553C9A text-size-6 lh-8 font-bold mt-2">
                {consecutiveDays?.days ?? 0}天
              </div>
            </div>

            <div className="rounded-2 bg-#F3F3FF p-4 flex-1">
              <div className="color-#4B5563 text-size-4 lh-5">待复习课程</div>
              <div className="color-#553C9A text-size-6 lh-8 font-bold mt-2">
                {upcomingReviews
            ...
            <div className="rounded-2 bg-#F3F3FF p-4 flex-1">
              <div className="color-#4B5563 text-size-4 lh-5">总完成数量</div>
              <div className="color-#553C9A text-size-6 lh-8 font-bold mt-2">
                {allStudyRecords?.length ?? 0}个
              </div>
            </div>
          </div>
          <Divider />
          ...
          <div className="bg-#fff rounded-3 p-6">
            ...
            <Tabs
              ...
              items={upcomingReviews.map((f) => {
                return {
                  label: dayjs(f.date).isSame(dayjs(), 'day') ? '今天' : f.date,
                  value: f.date,
                  key: f.date,
                  children: (
